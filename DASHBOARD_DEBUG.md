...
| 文件 | 大小 | 用途 |
|------|------|------|
| `font_zh_cn.c` | 16px 448KB | 默认中文 (zh_cn_font) |
| `font_zh_cn_14.c` | 14px 312KB | 小字中文 (zh_cn_14) |
| `font_zh_cn_16.c` | 16px 357KB | 中文 16px (font_zh_cn_16，未使用) |
| `font_zh_cn_18.c` | 18px 393KB | 大字中文 (font_zh_cn_18) |
...
| `car_ui.c/h` | 6 区域布局编排器 |
| `car_ui_sidebar.c/h` | 左侧圆形图标栏 (5 键: 电源/Vol+/Vol-/空调/菜单) |
| `car_ui_dashboard.c/h` | 中间仪表盘（图片+车速/油量/音量/转速叠加） |
| `car_ui_ac_panel.c/h` | 空调控制面板（温度/风量滑块 + 吹风模式按钮） |
| `app_menu_ui.c/h` | 应用菜单（2×2 网格: 音乐/视频入口） |
| `car_ui_status.c/h` | 右上时间天气卡片（1 秒刷新） |
| `car_ui_music_bar.c/h` | 右下迷你音乐栏（歌名/进度/播放控制） |
| `music_catalog.c/h` | 音乐目录扫描 (/mmcblk/music) |
| `music_controller.c/h` | 音乐播放核心（fork+pipe + 读写线程） |
| `music_player_ui.c/h` | 全屏音乐播放器（左控制 + 右列表） |
| `vp_config.h` | 视频播放器公共配置 |
| `vp_media_catalog.c/h` | 视频目录扫描 (/mmcblk/video) |
| `vp_playback_controller.c/h` | 视频播放引擎（fork+pipe + mplayer 管理） |
| `vp_playback_io_sync.c/h` | 视频 I/O 同步（信号 + 条件变量） |
| `vp_playback_monitor.c/h` | 视频状态解析（ANS_* → 进度/时长） |
| `vp_player_ui.c/h` | 视频播放器 UI（控制栏/进度条/列表） |
| `can_simulator.c/h` | CAN 数据模拟（驾驶循环状态机） |
| `font_zh_cn.c/h` | 多尺寸中文声明 (14/16/18px) |
| `font_icons_20.c/h` | 侧边栏图标字体 (Segoe MDL2) |
| `font_snowflake_26.c` | 雪花图标字体 (26px) |
| `font_symbols.txt` | 字体符号清单 (562 汉字) |
| `car_img_data.c` | 仪表盘背景图（440×460 BGRA 嵌入式） |
